...
`install.sh` 는 `SKILL.md` 를 가진 디렉토리만 스킬로 인식한다. 보조 파일은 스킬 디렉토리 아래 `references/`(필요할 때 읽는 문서)·`scripts/`(실행 코드)·`assets/`(산출물에 쓰는 파일)·`agents/`(서브에이전트 정의, 파일 하나가 에이전트 하나)에 두고 `SKILL.md` 에서 상대경로로 가리킨다 — symlink 설치라 경로가 유지된다. 저장소에서 사라진 스킬의 symlink 는 `install.sh` 가 걷는다 — 스킬 설명문끼리 발동 경쟁을 하므로, 지워진 스킬이 옛 이름으로 남아 있으면 안 된다.
