...
fast-forward 와 squash 를 둘 다 막는다. 머지 커밋이 안 생기면 브랜치명이 로그에서 통째로 사라져(실측: ff 이력에서 `ag/` 0건) 이니셜 접두어가 회고에 아무 값도 못 번다. squash 는 `scoped-commits` 가 **의미 단위**로 나눈 커밋을 하나로 뭉개서, 그 스킬의 전제("커밋 하나는 혼자 정당화되는 변경")를 무너뜨린다.
...
squash 를 쓰면 작성자가 PR 작성자로 보존되므로 `git log --author` 로 회고가 되고 이니셜이 필요 없어진다(실측 확인). 즉 그 경로를 고르면 **이 설계의 접두어 결정 전체가 무효가 된다.** 두 결정은 함께 서고 함께 무너진다.
