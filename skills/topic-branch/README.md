...
지워도 되는지를 가르는 조건이 둘이고, 그중 하나가 이름이다. `--merged` 단독이 왜 부족한지는 지시 옆에 남겨 뒀다 — [`references/cleanup.md`](references/cleanup.md) 가 그 문장을 든다. 그것만 A/B 에서 출력을 갈랐다.

표식이 될 만한 것이 둘인데 하나만 살아남는다. 워크트리 경로(`.claude/worktrees/` 아래)는 워크트리를 지우는 순간 함께 사라지고, 원격에는 애초에 안 따라간다. 이름은 브랜치가 지워진 뒤에도 머지 커밋에 남는다 — `--no-ff` 를 고른 것이 여기서 한 번 더 값을 낸다.

그래서 `git worktree remove` 가 기본 경로다. `ExitWorktree` 는 조건이 맞을 때의 지름길로만 둔다 — 그 조건과 그때 달라지는 절차는 [`references/cleanup.md`](references/cleanup.md) 가 든다. 정리는 브랜치를 만든 세션이 끝난 뒤의 일이라, 조건이 맞는 경우가 오히려 드물다.
