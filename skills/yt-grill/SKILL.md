...
**챕터가 토의의 단위다.** 3~5개로 나눈다. `transcript.md` frontmatter 의 `chapters`(유튜브 챕터)는 재료지 단위가 아니다 — 3분 영상에 9개, 20분에 10개라 그대로 쓰면 잘다(실측, [`README.md`](README.md)). 제목이 화제 전환을 알려 주니 묶어서 쓰고, 없으면 원문에서 화제가 바뀌는 곳으로 나눈다.
...
2. **판정한다.** `판정:` 에 도달/미도달과 회수를 적는다. 사용자가 주장을 제 말로 설명했으면 도달이다. 도달·미도달의 세부 정의는 두지 않는다 — 있어도 판정이 안 갈리고, 있으면 판정을 입 밖에 내는 판이 생겼다([`README.md`](README.md)).
   ...
   **판정은 머릿속에서만 한다.** "그건 영상 말 그대로네요" 라고 하는 순간 사용자가 말을 고르기 시작한다. 판정을 여는 자리는 챕터를 닫을 때뿐이다.
