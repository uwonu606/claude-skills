...
저장소 루트의 **`.scoped-commits` 마커 파일**이 적용 범위를 정한다. 마커가 없는 저장소에서는 커밋을 검사하지 않는다.

```bash
cd <이 컨벤션을 쓸 저장소>
touch .scoped-commits
```

훅은 전역(`~/.claude/settings.json`)에 한 번만 걸고, 저장소를 늘리는 것은 빈 파일 하나다. 저장소마다 훅 경로가 적힌 설정을 복제하면 그 복제본이 낡기 때문에 이렇게 했다.

마커를 커밋할지 `.gitignore` 에 넣을지는 저장소마다 정하면 된다. 커밋하면 그 저장소를 클론한 사람에게도 걸리고, 무시하면 나한테만 걸린다.

`git -C <경로> commit` 처럼 다른 저장소를 가리키는 경우 **cwd 가 아니라 그 대상 저장소**의 마커를 본다. 그러지 않으면 엉뚱한 저장소의 설정을 읽는다.
